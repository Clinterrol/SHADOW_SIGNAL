...
class HUD:
    ...
    def _draw_signal_bar(self, sig):
        ...
        pygame.draw.rect(self.screen, (50, 50, 50), (bar_x, bar_y, bar_w, bar_h), 1)
        pct = self.font_small.render(f"{int(sig.stability)}%", True, (80, 80, 80))
    ...
    def _draw_battery_bar(self, fl):
        ...
        color  = AMBER if fl.battery_pct > 0.5 else DANGER_RED
        if fill_w > 0:
            pygame.draw.rect(self.screen, color, (bar_x, bar_y, fill_w, bar_h))
        pygame.draw.rect(self.screen, (50, 50, 50), (bar_x, bar_y, bar_w, bar_h), 1)
        pct = self.font_small.render(f"{int(fl.battery)}%", True, (80, 80, 80))
        self.screen.blit(pct, (bar_x + bar_w + 6, bar_y))
    ...
    def _draw_health_bar(self, player):
        ...
        color  = (0, 180, 60) if hp_pct > 0.6 else AMBER if hp_pct > 0.3 else DANGER_RED
        if fill_w > 0:
            pygame.draw.rect(self.screen, color, (bar_x, bar_y, fill_w, bar_h))
        pygame.draw.rect(self.screen, (50, 50, 50), (bar_x, bar_y, bar_w, bar_h), 1)
    ...
    def _draw_flashlight_status(self, fl):
        color_map = {
            "ON": (0, 200, 80), "OFF": (60, 60, 60),
            "DEAD": (100, 0, 0), "OVERHEAT": (220, 80, 0),
            "UNSTABLE": AMBER
        }
    ...
    def _draw_blackout_warning(self):
        txt = self.font_large.render("// SIGNAL LOST //", True, DANGER_RED)
